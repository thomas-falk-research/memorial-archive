...
step1)
  ...
  hdr "NOW, IN THE UI"
  say "  Ingestion sources -> add source -> type PST -> path: /import/$(basename "$A")"
...
step2)
  ...
  hdr "NOW, IN THE UI"
  say "  Ingestion sources -> add source -> type PST -> path: /import/$(basename "$B")"
...
step3)
  ...
  if [ -z "$dupe_groups" ]; then
    ...
    say "  -> Import ONE generation per mailbox lineage instead: the newest law / historical /"
    say "     archive / personal / main file, and treat the rest as fallback if the hunt comes up dry."
  fi
  say ""
  say "  State: $STATE"
