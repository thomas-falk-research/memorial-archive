...
gate_ocr(){
  ...
  say ""
  say "  MEASURED on the box: the API base is ${c_b}/api/v1${c_0} through this same port, and it requires a"
  say "  session — /api/v1/search returns 401, /v1/search returns 404. So a scripted hunt is possible"
  say "  but needs a token; the UI already has one."
  say ""
  say "  To re-confirm the path on another install, read the STATUS, not just the body:"
