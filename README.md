...
> That last point used to be a comfortable falsehood. The globs were literally `*.sh ci/*.sh`, which
> silently excluded every script in a subdirectory — so `openarchiver/verify-openarchiver.sh` and
> `openarchiver/stage-mailbox.sh`, the two tools standing between a master mailbox and an app we do
> not fully trust, were the only code in the repo CI never parsed or linted. The enumerator now comes
> from `git ls-files`, and both gates were re-tested by planting a real parse error and a real
> shellcheck violation in a subdirectory script and confirming they **fail**.
