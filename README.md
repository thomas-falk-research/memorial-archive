...
It performs three static checks plus five end-to-end drills, and fails loudly (non-zero exit)
on any problem:
...
- **search + OCR drill** (`ci/search-roundtrip.sh`) — *"can the family actually find the will?"* It
  drives the **real `archive-index` / `archive-search` / `archive-find`** commands over a scratch
  archive seeded like the real one — copies from several labelled "devices", born-digital paperwork,
  and a committed **scanned-will fixture** (an image-only PDF, `ci/fixtures/will-scanned.pdf`, with no
  text layer) — and proves the acceptance test: the scanned will is found by its **OCR'd content** (a
  marker that exists only inside the image, plus the family's real queries — *will, testament,
  executor, beneficiary, estate, trust, probate, power of attorney*), born-digital documents on other
  devices are found, names are found by `archive-find`, and results span **multiple device labels**.
  It needs `recoll` + `tesseract` + `poppler-utils` (its own CI job installs them; `./ci/run.sh`
  self-skips it if they're absent). The fixture is regenerated by `ci/make-fixtures.sh`.
- **Paperless upgrade guard** (`ci/paperless-upgrade-guard.sh`) — a 7-case drill against stub
  `docker`/`curl`/`git` proving the v2→v3 migration hazard stays blocked.
- **Open Archiver preflight drill** (`ci/openarchiver-preflight-guard.sh`) — 26 cases against
  `openarchiver/preflight.sh`, the readiness check for the real ingestion, driven entirely by stub
  `docker`/`sudo`/`curl`/`hostname`/`tailscale`/`df`/`free`. Every condition it claims to detect is
  planted and must BLOCK, naming its reason: a dead backend behind a healthy frontend, a missing
  container, telemetry back on, the archive mounted into a container, a writable import mount, a
  dropped `STORAGE_TYPE`, deletion re-enabled, an OCR timeout too short for a multi-page fax, image
  drift, a link-local-only address, too little disk or RAM, and a `.env` backup that is real but
  **stale**.
- **Open Archiver OCR fixture drill** (`ci/openarchiver-ocr-fixture-guard.sh`) — the synthetic
  mailbox Gate 2 rests on, parsed back with Python's `mailbox`: each attachment byte-identical to
  its fixture, no marker token present as plaintext (or a hit would prove indexing, not OCR), and a
  missing format fixture announced rather than silently skipped.
- **Open Archiver secrets drill** (`ci/openarchiver-env-guard.sh`) — 22 cases against
  `openarchiver/backup-env.sh`, the tool that proves the unregenerable `ENCRYPTION_KEY` /
  `STORAGE_ENCRYPTION_KEY` are really backed up off the box. Every failure mode is exercised in the
  *failing* direction — the zero-byte file `sudo` leaves behind, a captured error message, a
  truncation, CRLF mangling, a missing or malformed key, a valid `.env` from a **different**
  install — and each case asserts the output names the right reason, not merely that the exit was
  non-zero. It also proves the tool never prints a secret, and that its output guard *fires* when
  one would otherwise be printed. Needs no tools beyond coreutils.
...
the checks enumerate **every `*.sh` tracked in the repo, at any depth** (`all_shell_scripts` in
`ci/lib.sh`) and every `/usr/local/bin/<cmd>` heredoc automatically.

> That last point used to be a comfortable falsehood, twice over. The globs were literally
> `*.sh ci/*.sh`, which silently excluded every script in a subdirectory — so
> `openarchiver/verify-openarchiver.sh` and `openarchiver/stage-mailbox.sh`, the two tools standing
> between a master mailbox and an app we do not fully trust, were the only code in the repo CI never
> parsed or linted. The first fix enumerated from `git ls-files`, which lists only **tracked** files —
> so a newly written script stayed invisible until it was committed, which is exactly backwards:
> the newest file is the likeliest to be wrong. That bug shipped and was caught here when a new
> script passed CI while untracked and failed the moment it was committed, having never been linted
> at all. The enumerator now uses `--cached --others --exclude-standard`, and both gates were
> re-tested by planting a real parse error and a real shellcheck violation in a subdirectory script
> and confirming they **fail**. Coverage: 33 scripts → 55.
